...
// BorkedTCPHandler implements Packager and Transporter interface.
type BorkedTCPHandler struct {
	borkedTCPPackager
	borkedTCPTransport
}

// NewBorkedTCPClient allocates a new BorkedTCPClient.
func NewBorkedTCPClient(address string) *BorkedTCPHandler {
	h := &BorkedTCPHandler{}
	h.Address = address
...
// borkedTCPPackager implements Packager interface.
type borkedTCPPackager struct {
...
//  Data: n bytes
func (mb *borkedTCPPackager) Encode(pdu *modbus.ProtocolDataUnit) (adu []byte, err error) {
...
func (mb *borkedTCPPackager) Verify(aduRequest []byte, aduResponse []byte) (err error) {
	// Transaction id
...
//  Length: 2 bytes
//  Unit identifier: 1 byte
func (mb *borkedTCPPackager) Decode(adu []byte) (pdu *modbus.ProtocolDataUnit, err error) {
...
// borkedTCPTransport implements Transporter interface.
type borkedTCPTransport struct {
	// Connect string
	Address string
...
func (mb *borkedTCPTransport) Send(aduRequest []byte) (aduResponse []byte, err error) {
	mb.mu.Lock()
	defer mb.mu.Unlock()
	...
	// Handle the malformed exception unique to Sungrow inverters using the winet-s v12
	if data[7] == 0x84 && svLen == 2 {
		if _, err = io.ReadFull(mb.conn, data[length:length+1]); err != nil {
			return
		}
...
func (mb *borkedTCPTransport) Connect() error {
	mb.mu.Lock()
	defer mb.mu.Unlock()
...
func (mb *borkedTCPTransport) startCloseTimer() {
	if mb.IdleTimeout <= 0 {
		return
	}
...
func (mb *borkedTCPTransport) Close() error {
	mb.mu.Lock()
	defer mb.mu.Unlock()
...
func (mb *borkedTCPTransport) flush(b []byte) (err error) {
...
func (mb *borkedTCPTransport) logf(format string, v ...interface{}) {
	if mb.Logger != nil {
...
func (mb *borkedTCPTransport) close() (err error) {
	if mb.conn != nil {
		err = mb.conn.Close()
		mb.conn = nil
	}
...
func (mb *borkedTCPTransport) closeIdle() {
	mb.mu.Lock()
	defer mb.mu.Unlock()
